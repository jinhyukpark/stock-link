...
import { Calendar, ChevronRight, Target, Zap, PieChart as PieChartIcon, Gauge, ArrowRightLeft, Download, Share2, Activity, Info, BarChart2, TrendingUp, Maximize2, Sparkles, X, ChevronLeft, ArrowUpRight, ArrowDownRight, Minus, AlertCircle, PieChart, LineChart as LucideLineChart } from "lucide-react";
...
const AnalysisBlock = ({ content, className }: { content: React.ReactNode, className?: string }) => {
  ...
  if (textContent.includes("긍정") || textContent.includes("반등") || textContent.includes("상승") || textContent.includes("유입") || textContent.includes("강세") || textContent.includes("우상향")) sentiment = "positive";
  if (textContent.includes("부정") || textContent.includes("하락") || textContent.includes("리스크") || textContent.includes("이탈") || textContent.includes("우려") || textContent.includes("약세") || textContent.includes("하향")) sentiment = "negative";
  ...
  return (
    <div className={cn("flex flex-col gap-4", className)}>
      <div className="bg-[#12141A] rounded-xl border border-white/5 overflow-hidden shadow-lg">
        ...
        <div className="p-5 border-b border-white/5">
          <div className="flex items-center gap-2 mb-3">
             <span className="text-rose-400 font-bold text-sm">한 줄 요약</span>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-white/5 bg-[#161A22]">
          <div className="p-4 flex flex-col gap-2">
            <span className="text-[11px] font-medium text-slate-500">단기 추세</span>
            <span className={cn(
              ...
              {sentiment === "positive" ? <><ArrowUpRight className="w-4 h-4"/> 우상향 곡선</> : sentiment === "negative" ? <><ArrowDownRight className="w-4 h-4"/> 하향 곡선</> : <><Minus className="w-4 h-4"/> 횡보</>}
            </span>
          </div>
          <div className="p-4 flex flex-col gap-2">
            <span className="text-[11px] font-medium text-slate-500">시장 분위기</span>
            <span className={cn(
              "text-[15px] font-bold",
              sentiment === "positive" ? "text-rose-400" : sentiment === "negative" ? "text-rose-400" : "text-amber-400"
            )}>
              {sentiment === "positive" ? "위험 선호" : sentiment === "negative" ? "위험 회피" : "눈치 보기"}
            </span>
          </div>
          <div className="p-4 flex flex-col gap-2">
            <span className="text-[11px] font-medium text-slate-500">권장 대응</span>
            <span className="text-[15px] font-bold text-slate-200">
      ...
      {/* Bottom Lightbulb Tip Area */}
      <div className="bg-blue-500/10 border-l-4 border-blue-500 p-4 flex items-start gap-3 shadow-sm rounded-r-lg">
