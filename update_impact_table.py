...
# Let's replace the whole Market Impact table.
# First, insert the grouping logic before the return statement of SocialAnalysisView component.
...
# Find the table in "3. ② 시장 영향도: 상세 분석"
pattern = r'<table className="w-full text-left min-w-\[1000px\] border-collapse">.*?</table>'
# Wait, let's use a simpler replace by finding the section.
start_str = """<SectionTitle icon={Activity} title="시장 영향도: 상세 분석" subtitle="언급된 종목들의 단기적 시장 영향 및 모멘텀 분석입니다" />"""
                                        ...
                                        <div className="flex flex-col gap-3">
                                            {group.stocks.map((stock: any, idx: number) => (
                                                <div key={idx} className="flex flex-col sm:flex-row sm:items-center gap-3 bg-slate-800/50 p-3 rounded-lg border border-white/5 hover:bg-slate-800 transition-colors">
                                                    ...
                                                    <div className="text-slate-300 text-xs leading-relaxed flex-1 border-l border-white/10 pl-4">
...
# Replace between SectionTitle and the closing div of that section
import re
content = re.sub(
    r'<SectionTitle icon=\{Activity\} title="시장 영향도: 상세 분석" subtitle="언급된 종목들의 단기적 시장 영향 및 모멘텀 분석입니다" />.*?</table>\s*</div>',
    new_table,
    content,
    flags=re.DOTALL
)
