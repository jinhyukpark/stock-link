...
const StockLogo = ({ ticker, name, className }: { ticker: string, name: string, className?: string }) => {
    ...
    else if (name === "KB금융") logoUrl = "https://logo.clearbit.com/kbfg.com";
    else if (name === "신한지주") logoUrl = "https://logo.clearbit.com/shinhangroup.com";
    else if (name === "한미반도체") logoUrl = "https://logo.clearbit.com/hanmisemi.com";
    else if (name === "NVIDIA" || name === "NVDA") logoUrl = "https://logo.clearbit.com/nvidia.com";
    else if (name === "HD현대중공업") logoUrl = "https://logo.clearbit.com/hdhyundai.com";
    else if (ticker && ticker.match(/^\\d{6}$/)) {
        logoUrl = `https://file.alphasquare.co.kr/media/images/stock_logo/kr/${ticker}.png`;
    } else if (ticker !== "N/A" && ticker !== "SPY" && ticker !== "QQQ" && ticker !== "XLF") {
        logoUrl = `https://logo.clearbit.com/${ticker.toLowerCase()}.com`;
    }

    const defaultClass = className || "w-8 h-8 rounded-lg";

    if (imgError || !ticker || ticker === "N/A" || !logoUrl || ticker === "SPY" || ticker === "QQQ" || ticker === "XLF") {
        return (
...
const Avatar = ({ name, className }: { name: string, className?: string }) => {
    ...
    } else if (name.includes("제롬 파월")) {
        imgUrl = "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a2/Jerome_Powell_official_portrait_%28cropped%29.jpg/220px-Jerome_Powell_official_portrait_%28cropped%29.jpg";
    } else if (name.includes("캐시 우드")) {
        imgUrl = "https://upload.wikimedia.org/wikipedia/commons/thumb/8/80/Cathie_Wood_%28cropped%29.jpg/220px-Cathie_Wood_%28cropped%29.jpg";
    } else if (name.includes("한국은행")) {
        imgUrl = "https://logo.clearbit.com/bok.or.kr";
    }
...
// 3. Direction Icons (Stock Market Style)
const BenefitIcon = () => (
...
const MOCK_DATA = {
    "2026-04-26": {
        highlights: [
            ...
            <><HighlightInfluencer>일론 머스크</HighlightInfluencer>, 테슬라 어닝콜에서 Capex 25% 상향 및 Optimus 대규모 양산 선언 → <TickerChip>삼성전자</TickerChip> <TickerChip>SK하이닉스</TickerChip> <HighlightPos>주목 집중</HighlightPos></>,
        ...
        speakers: [
            ...
            { 
                ...
                followers: "경제통계국",
                positiveStocks: [{ ticker: "005930", name: "삼성전자" }, { ticker: "000660", name: "SK하이닉스" }, { ticker: "069500", name: "코스피 전반" }],
                negativeStocks: [],
                stars: 3,
                time: "2026-04-23"
            },
...
const BenefitChip = ({ name, ticker }: { name: string, ticker: string }) => (
    <div className="flex items-center gap-1.5 bg-emerald-950 border border-emerald-700 text-emerald-300 rounded-md px-2 py-1 text-xs font-medium w-fit">
        <StockLogo ticker={ticker} name={name} className="w-4 h-4 rounded-sm object-cover bg-white" />
        <span>{name}</span>
    </div>
);

const RiskChip = ({ name, ticker }: { name: string, ticker: string }) => (
    <div className="flex items-center gap-1.5 bg-[#ff7c7e]/10 border border-[#ff7c7e]/40 text-[#ff7c7e] rounded-md px-2 py-1 text-xs font-medium w-fit">
        <StockLogo ticker={ticker} name={name} className="w-4 h-4 rounded-sm object-cover bg-white" />
        <span>{name}</span>
    </div>
);
...
export default function SocialAnalysisView() {
    ...
    return (
        <div className="space-y-4 pb-20 animate-in fade-in duration-500 max-w-[1400px] mx-auto relative overflow-hidden px-4 md:px-6">
            ...
            {/* 2. ① 주요 인사 발언 (상단) - NO ACCORDION, FULLY SEPARATED */}
            <section className="mb-16">
            ...
            {/* 3. ② 시장 영향 분석 (하단) - INDEPENDENT SECTION */}
            <section className="mb-16">
                        ...
                        return (
                            <div key={i} className={cn("p-6 rounded-xl border flex flex-col", bgClass)}>
                                <div className="flex items-start justify-between mb-4">
                                    ...
                                    <div className="mt-1">
                                        {isPos ? <BenefitIcon /> : isNeg ? <RiskIcon /> : <NeutralIcon />}
                                    </div>
                                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-emerald-950/20 border border-emerald-900/40 rounded-xl p-6 flex flex-col">
                        ...
                        <div className="pt-5 border-t border-emerald-900/30">
                            ...
                            <div className="flex gap-2 flex-wrap">
                                <TickerChip className="bg-emerald-950 text-emerald-300 border-emerald-600 px-3 py-1">삼성전자</TickerChip>
                                <TickerChip className="bg-emerald-950 text-emerald-300 border-emerald-600 px-3 py-1">SK하이닉스</TickerChip>
                                <TickerChip className="bg-emerald-950 text-emerald-300 border-emerald-600 px-3 py-1">한화오션</TickerChip>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6 flex flex-col">
                        ...
                        <div className="pt-5 border-t border-slate-700">
                            ...
                            <div className="flex gap-2 flex-wrap">
                                <TickerChip className="bg-slate-800 text-slate-300 border-slate-600 px-3 py-1">Tesla</TickerChip>
                                <TickerChip className="bg-slate-800 text-slate-300 border-slate-600 px-3 py-1">KB금융</TickerChip>
                                <TickerChip className="bg-slate-800 text-slate-300 border-slate-600 px-3 py-1">셀트리온</TickerChip>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-[#ff7c7e]/5 border border-[#ff7c7e]/20 rounded-xl p-6 flex flex-col">
                        ...
                        <div className="pt-5 border-t border-[#ff7c7e]/10">
                            ...
                            <div className="flex gap-2 flex-wrap">
                                <TickerChip className="bg-[#ff7c7e]/10 text-[#ff7c7e] border-[#ff7c7e]/40 px-3 py-1">현대차</TickerChip>
                                <TickerChip className="bg-[#ff7c7e]/10 text-[#ff7c7e] border-[#ff7c7e]/40 px-3 py-1">기아</TickerChip>
                                <TickerChip className="bg-[#ff7c7e]/10 text-[#ff7c7e] border-[#ff7c7e]/40 px-3 py-1">Apple</TickerChip>
                            </div>
                        </div>
                    </div>
                </div>
